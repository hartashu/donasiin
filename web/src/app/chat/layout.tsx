...
export default async function ChatLayout({
  ...
  return (
    <div className="flex h-screen w-full overflow-x-hidden bg-white">
      <div className="w-full md:w-1/3 max-w-full overflow-hidden border-r border-gray-200 flex flex-col">
        <ChatHeader />
        <ConversationList initialConversations={plainConversations} />
      </div>
      <main className="hidden md:flex flex-1 flex-col overflow-x-hidden overflow-y-hidden">
        {children}
      </main>
    </div>
  );
