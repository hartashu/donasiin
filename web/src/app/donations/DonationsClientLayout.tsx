...
export default function DonationsClientLayout({
    children,
}: {
    ...
    useEffect(() => {
        ...
        // Ensure the pathname for the push is the current category path
        const currentPath = categories.some(cat => cat.path === pathname) ? pathname : '/donations';
    ...
    useEffect(() => {
        ...
        if (showSearchBar) {
            timeout = setTimeout(() => setShowInput(true), 1700);
        } else {
            setShowInput(false);
        }
        ...
        <div className="min-h-screen flex flex-col bg-gray-50">
            ...
            {isDonationsRoute && (
                <div className={`md:hidden fixed inset-0 z-50 flex pointer-events-none`}>
                    <div
                        className={`fixed inset-0 bg-black bg-opacity-40 transition-opacity duration-300 ${showSidebar ? "opacity-100 pointer-events-auto" : "opacity-0"
                            }`}
                        onClick={() => setShowSidebar(false)}
                    />
                    <aside
                        className={`relative w-64 h-full bg-white p-4 shadow-lg transform transition-transform duration-300 ${showSidebar ? "translate-x-0" : "-translate-x-full"
                            }`}
                    >
