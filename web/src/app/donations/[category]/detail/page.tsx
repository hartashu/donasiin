...
export async function generateMetadata({ searchParams }: { searchParams: { slug?: string } }): Promise<Metadata> {
  // FIX: Pastikan ada 'await' di sini
...
export default async function DonationDetailPage({ searchParams }: { searchParams: { slug?: string } }) {
  // FIX: Pastikan ada 'await' di sini juga
  const { slug } = await searchParams;
