        ...
        <div className="flex flex-col h-full bg-white">
            <header className="flex items-center p-3 border-b border-gray-200 sticky top-0 bg-white z-10">
                ...
                <div className="w-10 h-10 rounded-full bg-gray-300 mr-3 relative">
                    {otherUser.avatarUrl ? (
                        <Image src={otherUser.avatarUrl} alt={otherUser.fullName || ''} fill className="rounded-full object-cover" />
                    ) : (
                        <div className="w-full h-full rounded-full bg-blue-500 flex items-center justify-center text-white font-bold text-base">
                            {otherUser.fullName?.charAt(0).toUpperCase()}
                        </div>
                    ...
                    {messages.map(msg => (
                        ...
                        >
                            <div className={`max-w-xs md:max-w-md p-3 rounded-2xl ${msg.senderId === currentUser.id
                                ...
                                <p className={`text-xs mt-1 text-right ${msg.senderId === currentUser.id ? 'text-green-200' : 'text-gray-400'
                                    }`}>
                                    {format(new Date(msg.createdAt), 'p')}
                                </p>
                            </div>
            ...
            <footer className="p-3 border-t border-gray-200 bg-white">
                <form onSubmit={handleSubmit} className="flex items-center gap-2">
                    ...
                    />
                    <button type="submit" disabled={!text.trim()} className="bg-green-800/80 text-white p-2 rounded-lg hover:bg-green-800/90 disabled:bg-gray-400">
                        <Send className="w-6 h-6" />
                    </button>
                </form>
            </footer>
