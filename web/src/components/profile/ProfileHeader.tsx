...
import { Edit, LogOut, FileText, ArrowDownToLine, ArrowUpFromLine, MapPin, CalendarDays, Clock } from "lucide-react";
...
export function ProfileHeader({ profile, stats, achievements, activityFeed, onEdit }: ProfileHeaderProps) {
    return (
        <div className="bg-white p-5 rounded-2xl shadow-lg w-full space-y-5">
            <div className="flex items-start gap-4">
                ...
                <div className="flex gap-2 flex-shrink-0">
                    <button onClick={onEdit} className="p-2 bg-gray-100 hover:bg-gray-200 rounded-full text-gray-600 transition-colors">
                        <Edit size={16} />
                    </button>
                    <button onClick={() => signOut({ callbackUrl: '/' })} className="p-2 bg-rose-50 hover:bg-rose-100 rounded-full text-rose-500 transition-colors">
                        <LogOut size={16} />
                    </button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 pt-4 border-t border-gray-100">
                ...
                <div className="col-span-2 pt-4 border-t border-gray-100 flex items-center justify-center gap-2 text-center">
                    ...
                    <p className="text-sm text-gray-600">
                        Daily Requests: <span className="font-bold text-gray-800">{stats.dailyRequestsMade} / {profile.dailyLimit}</span>
                    </p>
                </div>
            </div>
