...
//             <h2 className="text-2xl font-bold mb-6 text-gray-800">Ubah Biodata Diri</h2>
//             <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
//                 {errors.root && <div className="p-3 bg-red-100 text-red-700 text-sm rounded-md">{errors.root.message}</div>}

//                 <div className="flex items-center gap-4">
//                     <div className="relative">
//                         <Image src={preview || "/default-avatar.png"} width={80} height={80} alt="avatar" className="rounded-full object-cover border" />
...
//                     <input id="fullName" {...register("fullName")} disabled={isPending} className="w-full p-2 mt-1 border rounded-md focus:ring-brand focus:border-brand" />
//                     {errors.fullName && <p className="text-red-500 text-xs mt-1">{errors.fullName.message}</p>}
//                 </div>

//                 <div>
//                     <label htmlFor="username" className="block text-sm font-medium text-gray-700">Username</label>
//                     <input id="username" {...register("username")} disabled={isPending} className="w-full p-2 mt-1 border rounded-md focus:ring-brand focus:border-brand" />
//                     {errors.username && <p className="text-red-500 text-xs mt-1">{errors.username.message}</p>}
//                 </div>

//                 <div>
//                     <label htmlFor="address" className="block text-sm font-medium text-gray-700">Alamat</label>
//                     <textarea id="address" {...register("address")} disabled={isPending} rows={3} className="w-full p-2 mt-1 border rounded-md resize-none focus:ring-brand focus:border-brand" />
//                     {errors.address && <p className="text-red-500 text-xs mt-1">{errors.address.message}</p>}
//                 </div>

//                 <div className="flex justify-end gap-3 pt-4 border-t">
//                     <button type="button" onClick={onCancel} disabled={isPending} className="px-4 py-2 bg-gray-200 text-gray-800 rounded-md font-semibold hover:bg-gray-300">
//                         Cancel
//                     </button>
//                     <button type="submit" disabled={isPending} className="px-6 py-2 bg-brand text-white rounded-md font-semibold flex items-center gap-2 disabled:opacity-50">
