...
import { IRequestWithPostDetails, RequestStatus, IAuthor } from "@/types/types";
...
export function MyRequestsTab({
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-3">
            {requests.map((req) => {
                const author: IAuthor | undefined = req.postDetails.author;
                    ...
                    <div key={req._id} className="bg-white p-4 rounded-lg shadow-sm border">
                        <div className="flex justify-between items-start">
                            ...
                            {req.status === 'SHIPPED' && (
                                <button
                                    disabled={isPending}
                                    onClick={() => handleStatusUpdate(req._id, RequestStatus.COMPLETED)}
                                    className="bg-brand text-white px-3 py-1 rounded text-sm whitespace-nowrap"
                                >
                                    ✔️ Mark as Completed
                                </button>
                            )}
