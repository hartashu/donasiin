...
import { User, Plus, MessageCircle, House, Gift, LogOut, LogIn, LoaderCircle, Info, BookOpen } from "lucide-react";
...
const NavLink = ({ href, label, icon: Icon }: { href: string, label: string, icon: React.ElementType }) => {
    ...
    return (
        <Link
            href={href}
            className={`flex gap-2 items-center transition-colors ${isActive ? "text-gray-900" : "text-gray-500 hover:text-gray-900"}`}
        >
            ...
            <nav className="container mx-auto flex justify-between items-center p-3 px-4">
                ...
                <div className="flex gap-2 md:gap-3 items-center">
                    {isLoading ? (
                        ...
                        <>
                            <Link href="/chat" className="p-2 rounded-full hover:bg-gray-100 text-gray-600">
                                <MessageCircle className="w-5 h-5" />
                            </Link>
                            <Link href="/profile" className="p-2 rounded-full hover:bg-gray-100 text-gray-600">
